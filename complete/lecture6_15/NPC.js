...
// Pre-made Path-finding by someone - modified by me , added animations , shot interactions
class NPC {
  constructor(options) {
  ...
  initRifleDirection() {
    this.rifleDirection = {}; ///Pre-made by someone Rifle directions to hit
    ...
    this.rifleDirection.firingwalk = new THREE.Quaternion(
      -0.147,
      -0.04,
      0.784,
      0.6
  ...
  update(dt) {
        ...
        if (this.withinAggroRange()) {
          ...
          if (!this.isFiring) {
            ...
            if (this.isFiring && this.seeEnemy()) {
              ...
              const elapsedTime =
                this.app.clock.getElapsedTime() - this.bulletTime;
              if (elapsedTime > 3.0) this.newPath(this.randomWaypoint);
            }
          }
        } else {
          this.firing = false;
    ...
    if (this.calculatedPath && this.calculatedPath.length) {
      ...
      if (this.waypoints !== undefined && !this.aggro)
        this.newPath(this.randomWaypoint);
    }
  }
}
